...
Вся ключевая модель игры собрана в заголовке `types.h`. Это упрощает сопровождение и повторное использование кода.
...
Используется вместо `float Vector2` из Raylib, так как для работы с клетками удобнее работать используя целочисленные координаты.
...
#### Запуск игры
После успешной компиляции игру можно запустить напрямую.

Проект собирается как самодостаточный `.exe`, статически (или динамически) линкующийся с необходимыми компонентами Raylib согласно настройкам сборки. 
